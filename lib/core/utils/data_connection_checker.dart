...
class DataConnectionChecker {
  ...
  _maybeEmitStatusUpdate([Timer? timer]) async {
    ...
    if (_lastStatus != currentStatus && statusController.hasListener) {
      statusController.add(currentStatus);
    }

    // start new timer only if there are listeners
    if (!statusController.hasListener) return;
  ...
  // and the statusController's.onCancel event handler
  DataConnectionStatus? _lastStatus;
  Timer? _timerHandle;

  // controller for the exposed 'onStatusChange' Stream
  StreamController<DataConnectionStatus> statusController =
  ...
  Stream<DataConnectionStatus> get onStatusChange => statusController.stream;

  /// Returns true if there are any listeners attached to [onStatusChange]
  bool get hasListeners => statusController.hasListener;

  /// Alias for [hasListeners]
  bool get isActivelyChecking => statusController.hasListener;
}
